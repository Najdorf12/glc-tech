...
const Store = ({ theme, allProducts, arsPrice, setAllProducts }) => {
  ...
  // Divide los productos en segmentos de 10
      ...
      >
        ...
        <div className="w-full flex flex-col items-center mt-12 gap-y-6 xl:mt-24 xl:px-[6%]">
          {productChunks.map((chunk, index) => (
            <div key={index} className="w-full flex flex-wrap justify-center items-center gap-y-5 gap-x-3 sm:gap-x-4 lg:gap-x-9 lg:gap-y-14">
              {chunk.map((product, i) => (
                <Card key={i} product={product} arsPrice={arsPrice} />
              ))}
              {/* Renderiza una imagen después de cada segmento */}
              <div className="w-full flex justify-center my-8 lg:mb-12">
                {index % 2 === 0 ? (
                  <img src={imgAcoyteService} alt="Banner" className="w-full h-full object-contain object-center rounded-sm z-40 lg:w-[85%]" />
                ) : (
                  <img src={imgBannerXiaomi} alt="Banner" className="w-full h-full object-contain object-center rounded-sm z-40 lg:w-[85%]" />
                )}
              </div>
            </div>
          ))}
