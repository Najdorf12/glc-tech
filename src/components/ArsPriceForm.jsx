...
const ArsPriceForm = () => {
  ...
  const deleteArsPrice = (id) => {
    axios
      .delete(`https://glctech-backend.onrender.com/api/usdprice/${id}`)
      .catch((error) => console.error(error));
  };
  return (
    <section className="form-container mt-5">
      <form onSubmit={handleSubmit(submit)}>
        <div className="form-group">
          <label htmlFor="arsPrice">Peso Argentino - ACTUAL : {ARSPrice[0]?.usdPrice}</label>
        ...
        <div className="flex justify-between items-center mt-2">
          ...
          <button
            onClick={() => deleteArsPrice(ARSPrice[0]?._id)}
